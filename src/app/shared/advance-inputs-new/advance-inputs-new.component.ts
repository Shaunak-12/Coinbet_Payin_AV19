...
export class AdvanceInputsNewComponent {
  ...
  constructor(private fb:  FormBuilder) {
    ...
    //   value: [''], // Define the 'value' control explicitly
    // });
    // this.formGroup = this.fb.group({
    //   value: [''], // Parent form control
    //   subque: this.fb.array(
    //     this.question.subque.map(() => 
    //       this.fb.group({
    //         // Define your form controls for each subquestion
    //         value: [''], // Example
    //       })
    //     )
    //   ),
  ...
  getSubFormGroup(index: number): FormGroup {
  ...
  searchSelOps(val:string){
    ...
    let foundObj = this.question.options.filter(({ name }: { name: string }) => (name.toLocaleLowerCase()).includes(newVal));
    this.question.filteroptions = foundObj;
  }
  ...
  getFormControlValue(): string | null {
    const control = this.formGroup.get('value') as FormControl | null;
